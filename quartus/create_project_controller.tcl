# =============================================================================
# create_project_controller.tcl
#   Builds a SEPARATE Quartus Prime Pro project with the systolic engine
#   (matmul_controller) as the top-level entity, for per-engine
#   area/Fmax characterization.
# -----------------------------------------------------------------------------
# USAGE options:
# (a) Quartus Tcl Console:  cd <...>/quartus ; source create_project_controller.tcl
# (b) Quartus cmd prompt:   cd <...>/quartus ; quartus_sh -t create_project_controller.tcl
# (c) Tools > Tcl Scripts in the GUI.
# Then: File > Open Project > int8_matmul_controller.qpf, and compile.
#
# matmul_controller ports: clk, rst_n, start, a_in, b_in, c_flat, busy, done
# (no sel_systolic - exists only on matmul_top). clk stays a REAL pin.
...
project_new int8_matmul_controller -overwrite
...
#
# matmul_controller pulls in systolic_array to systolic_pe, and the package.
# Extra files (sequential/pe_mac/top) are harmless.
set_global_assignment -name TOP_LEVEL_ENTITY matmul_controller
...
export_assignments
project_close
puts "Project int8_matmul_controller created (top = matmul_controller)."
puts "Open int8_matmul_controller.qpf and run Start Compilation."
